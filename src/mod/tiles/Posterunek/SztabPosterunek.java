...
public class SztabPosterunek extends mod.Unit {
	...
	public void work(int direction){
		if(board[position].neighbours[direction]!=null && board[position].neighbours[direction].tile instanceof Unit && board[position].neighbours[direction].tile.getOwner()==this.owner ) 
			((Unit)board[position].neighbours[direction].tile).initiative.add(((Unit)board[position].neighbours[direction].tile).initiative.get(((Unit)board[position].neighbours[direction].tile).initiative.size()-1)-1); 
	}
	public void stopWork(){
		for(int i=0; i<6; i++){
			if(board[position].neighbours[i]!=null && board[position].neighbours[i].tile instanceof Unit && board[position].neighbours[i].tile.getOwner()==this.owner ) 
				((Unit)board[position].neighbours[i].tile).initiative.add(((Unit)board[position].neighbours[i].tile).initiative.get(((Unit)board[position].neighbours[i].tile).initiative.size()-1)-1); 
		}
	}
